<?php
  ...
  if(isset($_POST['vote-btn'])){
    ...
    $curr_vote = $result['votes'];
    $new_vote = $curr_vote + $vote;
    $time = date('y-m-d h:i:s');
    $upd = mysqli_query($conn,"UPDATE model_tbl SET votes = $new_vote WHERE id = $id");
    $insert = mysqli_query($conn,"INSERT INTO vote_log (model_id, name, votes, created_time) 
    VALUES ('$id', '$name', '$vote', '$time')");

    if($upd && $insert){
      $success = "<div class='alert alert-success'>You Just Voted for ".$result['full_name']." </div>";
    }
    else{
      $error_msg = "<div class='alert alert-danger'>Failed to vote! Network error</div>";
    }
  }
?>
...
<body class="homepage">
  ...
  <?php
    ...
    if(mysqli_num_rows($sql) > 0){
        echo '<section class="collection bg-light position-relative py-5">
        <div class="container">
        <div class="row">
            '.$success.'
            '.$error_msg.'
            ...
            <div class="col-md-6 column-container">
                ...
                <img src=admin/'.$result['pic'].' width="90%" alt="model" class="product-image img-fluid">
                </div>
            </div>
            <div class="col-md-6 column-container bg-white">
                <div class="collection-content p-5 m-0 m-md-5">
                <h3 class="element-title text-uppercase">Vote for me</h3>
                <p><a>Name : '.strtoupper($result['full_name']).' </a></p>
                <p>Nickname : '.strtoupper($result['username']).'</p>
                <p>Votes : '.$result['votes'].'</p>

                 <form method=post>
                    <label>No of Votes</label>:
                    <input type="number" placeholder  min="1" max="200" required class="form-control" name="vote"/>

                    <label>Your Name :
                    </label><input type="text" name="name" required class="form-control"/>

                    <input type="submit" class="btn btn-dark text-uppercase mt-3" id="vote-btn" name="vote-btn" value="Vote">
                </form>
